...
def xlsx_report_generator(sqlite_database, dir_out, file_name, write_multiple_files, out_extension, entries_table, dynamic_reports, dyn_rep_tab, anual_hist, full_hist):
    ...
    lista_consultas.append(["SELECT MesAno, sum (lg.Debito) as debitos , Sum (lg.Credito) as Creditos " \
                             f" FROM {entries_table} LG where LG.TIPO not in ('cartões de Crédito','Transf. Bco') " \
